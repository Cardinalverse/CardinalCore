...
test_that("qdiff works", 
{
	...
	expect_equal(
		qdiff(
			as.integer(c(0, NA, NA)), 
			as.integer(c(NA, 0, NA))),
		c(-Inf, Inf, 0))
	expect_equal(
		qdiff(
			as.double(c(0, NA, NA)), 
	...
	expect_equal(
		qdiff(
			as.integer(c(0, NA, NA)), 
			as.integer(c(NA, 0, NA)), relative=TRUE),
		c(-Inf, Inf, 0))
	expect_equal(
		qdiff(
			as.double(c(0, NA, NA)), 
			as.double(c(NA, 0, NA)), relative=TRUE),
		c(-Inf, Inf, 0))
...
test_that("bsearch works with doubles", 
{
	...
	expect_equal(
		bsearch(q, x), c(1, NA, 3, 5, NA))
	expect_equal(
		bsearch(q, x, nearest=TRUE), c(1, 3, 3, 5, 5))
	expect_equal(
		bsearch(3.0, x, tolerance=0), NA_integer_)
	expect_equal(
		bsearch(3.0, x, tolerance=0.5), 3)
	expect_equal(
		bsearch(3.0, x, tolerance=Inf), 3)
	expect_equal(
		bsearch(3.0, x, tolerance=0.1, relative=TRUE), NA_integer_)
	expect_equal(
		bsearch(3.0, x, tolerance=0.1, relative=TRUE, nearest=TRUE), 3)
	expect_equal(
		bsearch(3.0, x, tolerance=0.111, relative=TRUE), 3)
	expect_equal(
		bsearch(3.0, x, tolerance=0.111, relative=TRUE), 3)
	expect_equal(
